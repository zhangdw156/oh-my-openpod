...
`docker/<flavor>/docker-compose.yaml` 通过 `${IMAGE_VERSION:-local}` 消费这个版本。compose 本身不会自动读取 `VERSION`，因此在需要让本地 compose 构建的标签与中心版本一致时，应使用 `IMAGE_VERSION="$(cat VERSION)" docker compose ...` 这种前缀写法，或显式 `export IMAGE_VERSION="$(cat VERSION)"` 后再运行 compose；未设置时 `IMAGE_VERSION` 默认为 `local`。
