...
构建完成后，镜像会根据 `${IMAGE_VERSION:-local}` 进行打标，例如 `oh-my-openpod:${IMAGE_VERSION:-local}`。为了让本地 compose 构建的标签与仓库根的 `VERSION` 一致，应使用 `IMAGE_VERSION="$(cat VERSION)" docker compose ...` 这种前缀写法，或先执行 `export IMAGE_VERSION="$(cat VERSION)"` 再运行 compose；未设置时默认使用 `local`。
